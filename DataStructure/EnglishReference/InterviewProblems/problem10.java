...
Problem 10
    ...
    You are given a binary search tree, how you serialize and deserialize it? what kind of traversal works
    ...
    solution:      Three ordering serialization: 1) preorder; 2) inorder; 3) postorder.

    Deserialization: The process of converting a sequence of bits back into a tree data structure

    Solution:   If the sequence is in pre-order, then we can use it to reconstruct(deserialize) the whole tree without problems. If it's not in-order or post-order,
                We need to think about another way to reconstruct the tree...(Assuming it is possible)
...
class BinTree<E> {
  ...
  /** postorder tree walk: left, right, root */
   ...
   public BinTree<E> deserialize(BinNode<E>[] nodes) {
       
       if (nodes == null) {
           //zero nodes array, return an empty tree
           return null;
       }

       //assuming the nodes are in "pre-order" order.
       BinTree<E> bTree = new BinTree<E>();
       for (BinNode<E> node : nodes) {
           bTree.insert(node);
       }
...
public class problem10 {
   ...
   public static void main(String[] args) {
      ...
      */
      ...
      BinNode<Integer>[] nodes = new BinNode[13];
      ...
      bTree2 = bTree2.deserialize(nodes);
